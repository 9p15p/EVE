...
# Data options
parser.add_argument('--d16_path', default='/DAVIS-2017-trainval-480p')
parser.add_argument('--d17_path', default='/DAVIS-2017-trainval-480p')
...
parser.add_argument('--output', default='output/D17_val')
parser.add_argument('--save_all', action='store_true',
                    help='Save all frames. Useful only in YouTubeVOS/long-time video', )
parser.add_argument('--eval_on_dv17', help='use davis2017evaluation', action='store_true')
...
if "ok" in getpass.getuser():
    args.d16_path = "/data/ldz_proj/DATASET/DAVIS2017"
    args.d17_path = "/data/ldz_proj/DATASET/DAVIS2017"
    args.y18_path = "/data/ldz_proj/DATASET/YoutubeVOS2018"
    args.y19_path = "/data/ldz_proj/DATASET/YoutubeVOS2019"
elif "titan1" in getpass.getuser():
    args.d16_path = "/media/titan1/52f525b8-0ae0-4e4c-8c95-8c8641e2743a/ldz_proj/DATASET/DAVIS2017"
    args.d17_path = "/media/titan1/52f525b8-0ae0-4e4c-8c95-8c8641e2743a/ldz_proj/DATASET/DAVIS2017"
    args.y18_path = "/media/titan1/52f525b8-0ae0-4e4c-8c95-8c8641e2743a/ldz_proj/DATASET/YoutubeVOS2018"
    args.y19_path = "/media/titan1/52f525b8-0ae0-4e4c-8c95-8c8641e2743a/ldz_proj/DATASET/YoutubeVOS2019"
...
if is_youtube:
    ...
    if args.dataset == 'D16':
        ...
        if args.split == 'val':
            ...
            meta_dataset = DAVISTestDataset(path.join(args.d17_path), imset='2017/test-dev.txt', size=args.size)
        else:
            raise NotImplementedError
...
# Load our checkpoint
network = EVE(config, args.model).cuda().eval()
...
# sam = sam_model_registry[args.model_type](checkpoint=args.sam_checkpoint).cuda().eval()
...
for vid_reader in progressbar(meta_loader, max_value=len(meta_dataset), redirect_stdout=True):
    ...
    # # TODO:only for debug
    # if vid_reader.vid_name != 'blackswan':
    #     continue
...
if args.eval_on_dv17:
    time_start = time()
    print(f'Evaluating sequences for the semi-supervised task...')
    # Create dataset and evaluate
    dataset_eval = DAVISEvaluation(davis_root=args.d17_path, task='semi-supervised', gt_set=args.split)
    metrics_res = dataset_eval.evaluate(args.output)
    ...
    # Print the results
    sys.stdout.write(f"--------------------------- Global results for {args.split} ---------------------------\n")
    print(table_g.to_string(index=False))
    sys.stdout.write(f"\n---------- Per sequence results for {args.split} ----------\n")
