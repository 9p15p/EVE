...
from model.model_sam.trainer_sam import EVETrainer
from dataset.vos_dataset_sam import VOSDataset_SAM

from util.logger import TensorboardLogger
from util.util_sam.configuration_sam import Configuration
...
git_info = str(repo.active_branch) + ' ' + str(datetime.datetime.now().strftime('%b%d%H%M'))
...
for si, stage in enumerate(stages_to_perform):
    ...
    config = edict(**raw_config.args, **stage_config)
    ...
    if local_rank == 0:
        ...
        model = EVETrainer(config, logger=logger,
        ...
        model = EVETrainer(config, local_rank=local_rank, world_size=world_size).train()
    ...
    def renew_vos_loader(max_skip, finetune=False):
        ...
        davis_dataset = VOSDataset_SAM(
            path.join(davis_root, 'JPEGImages', '480p'),
            path.join(davis_root, 'Annotations', '480p'), max_skip, is_bl=False,
            subset=load_sub_davis(), num_frames=config['num_frames'], finetune=finetune, len_side=1024)
        if config['close_ytb']:
            print("not use ytbvos.")
            train_dataset = ConcatDataset([davis_dataset] * 5)
        else:
            yv_dataset = VOSDataset_SAM(
                path.join(yv_root, 'JPEGImages'),
                path.join(yv_root, 'Annotations'), max_skip // 5, is_bl=False,
                subset=load_sub_yv(), num_frames=config['num_frames'], finetune=finetune, len_side=1024)
    ...
    def renew_bl_loader(max_skip, finetune=False):
        train_dataset = VOSDataset_SAM(path.join(bl_root, 'JPEGImages'),
                                   path.join(bl_root, 'Annotations'), max_skip, is_bl=True,
                                   num_frames=config['num_frames'], finetune=finetune, len_side=1024)
    ...
    if stage == '0':
        ...
        yv_root = path.join(path.expanduser(config['yv_root']))
