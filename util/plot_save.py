...
class FeatureVisualizer:
    ...
    def __call__(self, output, input_={}, save=True, save_RGB=True):
        ...
        """
        if output.shape[-1] == 3 and save_RGB:
            output = torch.tensor(output)
            output = output.permute(2, 0, 1)[None]
            ...
            for tag in data:
                ...
                if tag == 'input_i':
                    ...
                    if c == 3 and save_RGB:
                        ...
                        out_img = np.zeros((cc * h, cc * w)).astype(float)
...
if __name__ == "__main__":
    msk = cv2.imread(
        "/media/titan1/52f525b8-0ae0-4e4c-8c95-8c8641e2743a/ldz_proj/DATASET/DAVIS2017/JPEGImages/480p/aerobatics/00000.jpg").transpose(
        (2, 0, 1))[None, ::-1, :]
    vis = FeatureVisualizer()
    vis(msk, save_RGB=False)
    vis(msk)
