...
"""
相较原版关闭RandomResizedCrop
"""

class VOSDataset_SAM(Dataset):
    """
    ...
    def __init__(self, im_root, gt_root, max_jump, is_bl, subset=None, num_frames=3, max_num_obj=3, finetune=False, len_side=384):
        ...
        # im_lone: used in img alone. Mainly for color or brightness
        # im_dual/gt_dual: used both in im and gt, and transformed in the same way. Mainly for geometry
        # pair: seeds are different among all frames.
        # all:  seeds are the same among all frames.
        ...
        if self.is_bl:
            ...
            self.all_im_dual_transform = transforms.Compose([
                ...
                transforms.RandomResizedCrop((self.len_side, self.len_side), scale=(0.25, 1.00),
            ...
            self.all_im_dual_transform = transforms.Compose([
                transforms.RandomHorizontalFlip(),
                transforms.RandomResizedCrop((self.len_side, self.len_side), scale=(0.36, 1.00),
                                             interpolation=InterpolationMode.BILINEAR)
            ])
    ...
    def __getitem__(self, idx):
        ...
        while trials < 5:
            ...
            acceptable_set = set(range(max(0, frames_idx[-1] - this_max_jump),
        ...
        cls_gt = np.zeros((self.num_frames, self.len_side, self.len_side), dtype=np.int64)
        first_frame_gt = np.zeros((1, self.max_num_obj, self.len_side, self.len_side), dtype=np.int64)
