...
```bash
python -m torch.distributed.run --master_port 25763 --nproc_per_node=2 train_sam.py --exp_id retrain --stage 03
```
...
```bash
python -m torch.distributed.launch --master_port 25763 --nproc_per_node=2 train_sam.py --exp_id retrain_stage3_only --stage 3 --load_network saves/XMem-s0.pth
```
