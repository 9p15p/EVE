...
class FeatureFusionBlock(nn.Module):
    ...
    def forward(self, x, g):  # use 'g' to produce 'r', then block2(g+r)
...
class ValueEncoder(nn.Module):
    def __init__(self, value_dim, hidden_dim, single_object=False, f16_ch=1024):
        super().__init__()
        ...
        self.distributor = MainToGroupDistributor()
        self.fuser = FeatureFusionBlock(f16_ch, 256, value_dim, value_dim)  # 256==self.layer3's out_channels
        if hidden_dim > 0:
    ...
    def forward(self, image, image_feat_f16, h, masks, others, is_deep_update=True):
        ...
        if not self.single_object:
            ...
            g = masks.unsqueeze(2)
        g = self.distributor(image, g)  # 合并img和g，[bz, num_obj, C, H, W]

        batch_size, num_objects = g.shape[:2]
        g = g.flatten(start_dim=0, end_dim=1)  # [bz*num_obj, C, H, W]
        ...
        g = self.fuser(image_feat_f16, g)  # 融合img特征和mask特征g
...
class Decoder(nn.Module):
    ...
    def forward(self, f16, f8, f4, hidden_state, memory_readout, h_out=True):
        ...
        # return hidden_state, logits, g4  # g4 for cka_kd
